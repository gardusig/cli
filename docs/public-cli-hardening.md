...
| [#62](https://github.com/gardusig/python-cli/issues/62) | `cli docker` | **Pass** | JSON output, filters, write gates, contest runner lifecycle in `docs/docker.md`, `docker_integration` checks. |
| [#63](https://github.com/gardusig/python-cli/issues/63) | `cli gh` / `cli project` | **Pass** | GH CRUD + policy in `cli_api_checks`; top-level `project_integration` checks for list/spawn/pairs/deploy/link. Raw `cli gh project` remains policy-blocked. |

...
1. Run `cli git large files --worktree -n 30` on a dirty tree before commit.
2. Add offenders to `.gitignore` or remove them.
3. Re-run on tracked files only to confirm the index is lean.

Related: [Git commands](git.md) · [Quick defaults](quick-defaults.md) · `cli links`

...
smoke_contains "0.1.0" python -m cli --version

smoke_contains "Repository:" python -m cli drive status
smoke_contains "restore: not implemented yet" python -m cli restore
smoke_contains "upload" python -m cli drive --help
smoke_contains "ingest" python -m cli notion --help
smoke_contains "bookmarks" python -m cli chrome --help
smoke_contains "deploy" python -m cli notion --help

links_out="$(mktemp)"
python -m cli links >"$links_out" 2>&1
...
(
  cd "$tmpdir/repo"
  smoke_contains "smoke-branch" python -m cli git start smoke-branch --no-prep

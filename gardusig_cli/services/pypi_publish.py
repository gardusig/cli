...
from gardusig_cli.utils.config import project_root
...
def sync_version_files(root: Path | None, version: str) -> None:
    """Write ``version`` to pyproject.toml and gardusig_cli/__init__.py before a release build."""
    root = (root or project_root()).resolve()
    ...
    init_py = root / "gardusig_cli" / "__init__.py"
    ...
    if init_count != 1:
        raise PyPiPublishError("failed to update __version__ in gardusig_cli/__init__.py")

...
def endpoint_checks() -> list[EndpointCheck]:
    ...
    checks: list[EndpointCheck] = [
        ...
        EndpointCheck("git large files", ("git", "large", "files", "-n", "1"), needle="cli/"),
        EndpointCheck(
            "git branch current",
            ("git", "branch", "current"),
            needle="main",
...
def run_endpoint_check(
    ...
    *,
    ...
    if check.failure == "missing_notion_token":
        ...
        token_patch = patch(
            "gardusig_cli.commands.notion.require_notion_token",
        ...
        with patch_remote_git():
            for check in endpoint_checks():
                ...
                if check.failure == "review_fail":
                    review_patch = patch("gardusig_cli.commands.git.run_review", return_value=1)
                with review_patch:

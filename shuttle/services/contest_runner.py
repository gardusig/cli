...
from shuttle.services.contest_docker import (
    ContestDockerConfig,
    RunOutcome,
    RunStatus,
    classify_compile,
...
from shuttle.services.contest_serde import compare_text, unified_diff
from shuttle.utils.config import load_yaml, project_root
...
class ContestValidateOptions:
    ...
    timeout: float = 10.0
    memory_mb: int = 256
    image: str = "shuttle-contest:runner"
    cxx_std: str = "c++17"
...
def resolve_options(
    *,
    ...
    return ContestValidateOptions(
        ...
        image=str(pick("image", image, "shuttle-contest:runner")),
...
def validate_contest(options: ContestValidateOptions) -> ContestValidateResult:
    ...
    with tempfile.TemporaryDirectory(prefix="shuttle-contest-") as tmp:
        workspace = Path(tmp)
        try:

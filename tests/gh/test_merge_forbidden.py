"""GitHub policy — blocked merge, projects, rulesets."""
...
def test_gh_provider_allows_pr_merge_break_glass(monkeypatch) -> None:
    ...
    # Will fail on actual gh call — policy must not block first
    with pytest.raises(Exception) as exc:
        provider.run(["pr", "merge", "1"], check=False)
    assert not isinstance(exc.value, MergeForbiddenError)
...
def test_cli_gh_project_list_blocked() -> None:
    runner = CliRunner()
    result = runner.invoke(app, ["gh", "project", "list"])
    assert result.exit_code == 1
    assert "projects blocked" in (result.output + (result.stderr or "")).lower()
...
def test_check_gh_args_project() -> None:
...
def test_blocked_catalog_covers_merge_and_projects() -> None:
    ids = {row["id"] for row in blocked_operations_catalog()}
    assert {"pr-merge", "project", "ruleset"} <= ids

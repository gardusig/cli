...
from src.services.notion_sync import NotionSyncResult
...
def test_notion_pairs_build(monkeypatch, tmp_path: Path) -> None:
    ...
    with (
        patch("src.commands.notion.notion_task_root", return_value=task_root),
        patch("src.commands.notion.notion_pairs_file", return_value=task_root / "tasks.pairs.json"),
        patch(
            "src.commands.notion.build_pairs_manifest",
            return_value=NotionSyncResult(processed=1),
        ),
        patch("src.commands.notion.load_pairs", return_value=[]),
        patch(
            "src.commands.notion.pair_deploy_rollout",
...
def test_notion_deploy_success_with_warnings(monkeypatch, tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("src.commands.notion.notion_pairs_file", return_value=manifest),
    ):
        mock_cfg.return_value.notion.database_id = "db-123"

...
def test_resolve_gh_command_path_to_gh_package() -> None:
    packages = resolve_test_packages(["src/commands/gh.py"])

    assert "gh" in packages


def test_resolve_test_path_to_package() -> None:
    packages = resolve_test_packages(["tests/gh/test_commands.py"])

    assert "gh" in packages


def test_absolute_paths_are_normalized() -> None:
    packages = resolve_test_packages([str(ROOT / "src" / "commands" / "gh.py")])
...
def test_registry_paths_are_portable_repo_relative_paths() -> None:
    ...
    assert (
        normalize_changed_path("/tmp/other-checkout/src/commands/gh.py")
        == "tmp/other-checkout/src/commands/gh.py"
    )
...
def test_package_command_payload_uses_registry_paths() -> None:
    payload = package_command_payload("gh")

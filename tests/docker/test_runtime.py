"""Unit tests for cli.services.docker_runtime."""
...
@patch("src.services.docker_runtime.shutil.which", return_value=None)
def test_run_docker_missing_binary(_which: MagicMock) -> None:
    from src.services.docker_runtime import run_docker

    with pytest.raises(RuntimeError, match="PATH"):
        run_docker(["ps"])


@patch("src.services.docker_runtime.shutil.which", return_value="/usr/bin/docker")
def test_run_docker_raises_on_failure(_which: MagicMock) -> None:
    from src.services.docker_runtime import run_docker

    with patch("src.services.docker_runtime.subprocess.run") as mock_proc:
...
def test_docker_available_true(_which: MagicMock) -> None:
    from src.services.docker_runtime import docker_available
...
def test_system_df(mock_run: MagicMock) -> None:
    from src.services.docker_runtime import system_df
...
def test_prune_images_dangling(mock_run: MagicMock) -> None:
    from src.services.docker_runtime import prune_images
...
def test_prune_build_cache(mock_run: MagicMock) -> None:
    from src.services.docker_runtime import prune_build_cache
...
def test_remove_containers_named(mock_run: MagicMock) -> None:
    from src.services.docker_runtime import remove_containers
...
def test_list_containers_running_only(mock_run: MagicMock) -> None:
    from src.services.docker_runtime import list_containers

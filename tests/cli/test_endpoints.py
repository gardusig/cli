...
@patch("src.commands.git.run_review", return_value=0)
...
@patch("src.commands.git.run_review", return_value=1)
...
def test_git_branch_delete_merged_with_yes(mock_delete: MagicMock, snapshot: MagicMock) -> None:
    with _mock_snapshot(snapshot):
        result = runner.invoke(app, ["git", "branch", "delete", "--merged", "--yes"])
    assert result.exit_code == 0
    assert "deleted 2 branch(es)" in result.stdout
    mock_delete.assert_called_once_with(yes=True)


@patch.object(GitShortcuts, "branch_delete_all", return_value=["a", "origin/b"])
@patch.object(GitShortcuts, "remote_branch_names", return_value=["b"])
@patch.object(GitShortcuts, "local_branch_names", return_value=["a"])
